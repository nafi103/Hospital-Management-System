...
namespace HospitalManagementSystem.Controllers
{
    ...
    public class DoctorDashboardController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            // The latest CaseSummary draft per patient (whatever its verdict), so each
            // consultation card can show its own pre-visit AI review panel inline instead
            // of a separate list elsewhere on the page - still-Pending drafts get
            // Accept/Edit/Reject, already-reviewed ones show who reviewed it and when, and
            // only a patient with no suggestion at all falls back to the empty/loading
            // states. Explicitly scoped to CaseSummary: a patient can also have
            // PatientInstructions suggestions (generated from one of their prescriptions,
            // reviewed on the Prescriptions page, not here) - without this filter, this
            // panel would show whichever suggestion happened to be newest, regardless of
            // type. Grouped in memory (not translated to SQL) - the active-consultation
            // list is always small, so this is cheap and avoids EF's spotty support for
            // "top 1 per group" queries.
